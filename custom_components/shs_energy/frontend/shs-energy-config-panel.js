...
class ShsEnergyConfigPanel extends HTMLElement {
  ...
  async _downloadVerification() {
    try {
      ...
      const a = document.createElement("a"); a.href = url; a.download = "shs-control-verification.json.gz"; a.click(); URL.revokeObjectURL(url);
  ...
  _renderSchedule() {
    ...
    return `<div class="card"><div class="status-heading"><h2>Your schedule</h2>${this._refreshError ? this._statusBadge("unavailable", "Status unconfirmed") : this._statusBadge(status.state, status.label)}</div>
      <p>${this._escape(this._refreshError ? "Current status could not be confirmed. The last received status may be stale." : status.reason)}</p><p class="muted">Issued ${this._time(status.issued_at)} · Instructions until ${this._time(status.binding_until)}</p>
      ...
      <div class="card"><h2>Control verification</h2><p>Download one file containing all devices, plan slots, exact service requests and operation coverage. Verification does not test physical response. The file contains local entity IDs and configuration. Repeated checks are grouped with counts and first/latest observations. The latest 2,000 decision groups are retained; plan slots are stored once. Downloads are gzip-compressed JSON.</p><button class="secondary" data-action="verification">Download control verification</button></div>
