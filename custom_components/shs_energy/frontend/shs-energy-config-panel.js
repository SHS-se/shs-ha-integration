...
class ShsEnergyConfigPanel extends HTMLElement {
  ...
  _renderSchedule() {
    ...
    return `<div class="card"><div class="status-heading"><h2>Your schedule</h2>${this._refreshError ? this._statusBadge("unavailable", "Status unconfirmed") : this._statusBadge(status.state, status.label)}</div>
      ...
      <details class="compact"><summary>Plan details</summary><p>Full plan ID: <code>${this._escape(status.plan_id)}</code></p><p>Compare this ID with the website to identify the same plan. Each replacement has a new ID.</p><p>SHS normally checks for a new plan every 15 minutes. The cached schedule can continue until it expires if a replacement does not arrive.</p></details>` : ""}<p class="muted">Published prices until ${this._time(status.binding_until)} · Plan valid until ${this._time(status.valid_until)}</p>
