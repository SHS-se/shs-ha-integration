...
class ShsEnergyConfigPanel extends HTMLElement {
  ...
  async _downloadVerification() {
    try {
      ...
      this._notice = `Downloaded controller diagnostics for ${data.current.devices.length} devices, with ${data.evaluations.length} runtime groups and ${data.attempts.length} verification groups.`;
    } catch (error) { this._error = error?.message || String(error); }
    this._render();
  }
