...
test('laundry editor hides empty alternatives but retains populated fields and required dependencies', () => {
  ...
  const html = panel._fields([{ key: 'temperature', label: 'Room temperature', kind: 'entity', required: true },
    ...
    { key: 'mode_entity_id', label: 'Mode', kind: 'entity' },
    { key: 'offset_minimum', label: 'Lowest offset', kind: 'number' }], { temperature: 'sensor.room', offset_entity_id: 'number.offset' });
  assert.match(html, /sensor.room/); assert.doesNotMatch(html, /Heating permission|data-field-key="mode_entity_id"/);
  assert.match(html, /data-field-key="offset_minimum"/); assert.match(html, /aria-label="Room temperature"/);
});
